...
class ImprovedNLPFlow2(FlowSpec):
    ...
    def preprocess(self):
        ...
        bag_of_words = vectorizer.fit_transform(processed_reviews).toarray()
        ...
        self.x_train, self.x_val, self.y_train, self.y_val, _, self.review_val = \
            train_test_split(bag_of_words, self.df['label'],self.df['review_text'], test_size=self.split_size,random_state=2023)
    ...
    def modeling(self):
        ...
        self.model = model
        self.pred_val = model.predict(self.x_val)
        self.prob_val = model.predict_proba(self.x_val)[:,0]
        self.acc = accuracy_score(self.y_val, self.pred_val)
        self.rocauc = roc_auc_score(self.y_val, self.prob_val)
    ...
    def end(self):
        ...
        print(msg.format(round(self.acc, 3), round(self.rocauc, 3)))

        current.card.append(Markdown("# Womens Clothing Review Results"))
        current.card.append(Markdown("## Overall Accuracy"))
        current.card.append(Artifact(self.acc))
        current.card.append(Markdown("## ROC AUC"))
        current.card.append(Artifact(self.rocauc))

        current.card.append(Markdown("## Top words and their coefficients"))
        ...
        if np.sum(false_pos) > 0:
            fp_reviews = self.review_val.loc[false_pos].sample(2)
        ...
        if np.sum(false_neg) > 0:
            fn_reviews = self.review_val.loc[false_neg].sample(2)
            current.card.append(Markdown('\n\n'.join(fn_reviews)))
        else:
            current.card.append(Markdown('none'))
